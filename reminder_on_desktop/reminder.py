...
def get_user_data_dir(app_dir: Path) -> Path:
    data_root = os.environ.get("APPDATA") or os.environ.get("LOCALAPPDATA")
    if data_root:
        return Path(data_root) / "google_calendar_on_desktop"
    return app_dir


APP_DIR = get_app_dir()
USER_DATA_DIR = get_user_data_dir(APP_DIR)
CONFIG_PATH = APP_DIR / "reminder_widget_config.json"
LOCAL_REMINDERS_PATH = USER_DATA_DIR / "reminder_local_events.json"
LEGACY_LOCAL_REMINDERS_PATH = APP_DIR / "reminder_local_events.json"
...
def local_reminder_paths() -> list[Path]:
    paths = [LOCAL_REMINDERS_PATH]
    if LEGACY_LOCAL_REMINDERS_PATH != LOCAL_REMINDERS_PATH:
        paths.append(LEGACY_LOCAL_REMINDERS_PATH)
    return paths
...
def load_local_reminders(tz) -> list[ReminderEvent]:
    ...
    for reminder_path in local_reminder_paths():
        if not reminder_path.exists():
            continue

        try:
            payload = json.loads(reminder_path.read_text(encoding="utf-8-sig"))
        ...
        for raw_item in raw_items:
            ...
            reminder_id = clean_text(raw_item.get("id"), uuid.uuid4().hex)
...
def save_local_reminders(reminders: list[ReminderEvent]) -> None:
    ...
    LOCAL_REMINDERS_PATH.parent.mkdir(parents=True, exist_ok=True)
    temp_path = LOCAL_REMINDERS_PATH.with_name(f"{LOCAL_REMINDERS_PATH.name}.tmp")
    temp_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
    temp_path.replace(LOCAL_REMINDERS_PATH)
...
class ReminderApp:
    ...
    def is_active_period_event(self, event: ReminderEvent) -> bool:
        return event.end > self.period_start and event.start < self.period_end

    def is_sticky_local_reminder(self, event: ReminderEvent) -> bool:
        return event.source == "local" and event.all_day

    def is_event_hidden(self, event: ReminderEvent) -> bool:
        return (
            self.event_hidden_key(event) in self.hidden_event_keys
            or event.id in self.hidden_event_keys
        )

    def is_completed_sticky_reminder(self, event: ReminderEvent) -> bool:
        return (
            self.is_sticky_local_reminder(event)
            and self.is_event_hidden(event)
            and event.start.date() < datetime.now(self.tz).date()
        )

    def should_show_event(self, event: ReminderEvent) -> bool:
        if self.is_completed_sticky_reminder(event):
            return False
        return self.is_active_period_event(event) or self.is_sticky_local_reminder(event)

    def event_display_day(self, event: ReminderEvent) -> date:
        if self.is_sticky_local_reminder(event):
            return event.start.date()
        return max(event.start.date(), self.period_start.date())
    ...
    def render_agenda(self) -> None:
        ...
        for event in events:
            day = self.event_display_day(event)
    ...
    def should_mask_event(self, event: ReminderEvent) -> bool:
        return event.private or self.is_event_hidden(event)
    ...
    def save_dialog_reminder(
        ...
        dialog.destroy()
        self.render()
        self.scroll_to_day(self.event_display_day(reminder))
